...
const BillingQuantityEntry = () => {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="ml-64 pt-16">
        <div className="p-8">
          <Button variant="ghost" className="mb-6 -ml-2 text-muted-foreground hover:text-foreground" onClick={() => navigate(`/projects/${projectId}/orders/${orderId}`)}>
              ...
              <CollapsibleContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                  <div>
                    ...
                    <Input value={measureLabels.measure1} onChange={e => setMeasureLabels({
                    ...measureLabels,
                  ...
                  <div>
                    ...
                    <Input value={measureLabels.measure2} onChange={e => setMeasureLabels({
                    ...measureLabels,
                  ...
                  <div>
                    ...
                    <Input value={measureLabels.measure3} onChange={e => setMeasureLabels({
                    ...measureLabels,
                    measure3: e.target.value
          ...
          <Card className="p-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="relative flex-1">
                <Input placeholder="Search by item description..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    ...
                    {item?.milestones?.map((milestone: any, index: number) => <TableHead key={index} className="w-[120px] font-semibold text-right">
                ...
                <TableBody>
                  {filteredEntries.map((entry, index) => <TableRow key={entry.id} className="hover:bg-muted/30">
                      <TableCell className="text-center text-muted-foreground">{index + 1}</TableCell>
                      <TableCell>
                        <Input value={entry.itemDescription} onChange={e => updateEntry(entry.id, "itemDescription", e.target.value)} placeholder="Enter description..." className="border-0 bg-transparent" />
                      </TableCell>
                      <TableCell>
                        <Input value={entry.area} onChange={e => updateEntry(entry.id, "area", e.target.value)} placeholder="Area..." className="border-0 bg-transparent" />
                      </TableCell>
                      <TableCell>
                        <Input type="number" step="0.01" value={entry.length || ""} onChange={e => updateEntry(entry.id, "length", e.target.value)} className="border-0 bg-transparent text-center" />
                      </TableCell>
                      <TableCell>
                        <Input type="number" step="0.01" value={entry.breadth || ""} onChange={e => updateEntry(entry.id, "breadth", e.target.value)} className="border-0 bg-transparent text-center" />
                      </TableCell>
                      <TableCell>
                        <Input type="number" step="0.01" value={entry.height || ""} onChange={e => updateEntry(entry.id, "height", e.target.value)} className="border-0 bg-transparent text-center" />
                      </TableCell>
                      <TableCell>
                        <Input type="number" step="0.01" value={entry.qty || ""} onChange={e => updateEntry(entry.id, "qty", e.target.value)} className="border-0 bg-transparent text-center" />
                      </TableCell>
                      <TableCell className="text-right font-semibold text-primary">
                        {entry.total.toFixed(2)}
                      </TableCell>
                      {item?.milestones?.map((milestone: any, index: number) => <TableCell key={index} className="text-right font-medium text-muted-foreground">
                          {(entry.total * milestone.percentage / 100).toFixed(2)}
                        </TableCell>)}
                      <TableCell className="text-center">
                        <Button variant="ghost" size="sm" onClick={() => deleteRow(entry.id)} className="text-destructive hover:text-destructive">
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </TableCell>
                    </TableRow>)}
                  
                  {Object.entries(subtotalsByArea).map(([area, total]) => <TableRow key={`subtotal-${area}`} className="bg-muted/30 font-semibold">
                      ...
                      {item?.milestones?.map((milestone: any, index: number) => <TableCell key={index} className="text-right text-blue-600">
                  ...
                  <TableRow className="bg-muted/50 font-bold">
                    ...
                    {item?.milestones?.map((milestone: any, index: number) => <TableCell key={index} className="text-right text-primary text-lg">
